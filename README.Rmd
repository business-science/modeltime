...
No need to switch back and forth between various frameworks. `modeltime` unlocks machine learning & classical time series analysis.
  ...
  - __forecast__: Use ARIMA, ETS, and more models coming. 
  - __prophet__: Use Facebook's `prophet()` algorithm
  - __tidymodels__: Use any `parsnip` model: `rand_forest()`, Boosted `boost_tree()`, `linear_reg()`, `mars()`, `svm_rbf()` to forecast 
