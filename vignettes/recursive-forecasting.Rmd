...
-   Add fitted models to a **Model Table** using `modeltime_table()`.

-   Perform **Forecast Evaluation** using `modeltime_forecast()` and `plot_modeltime_forecast()`.
...
set.seed(123)
model_fit_lm_recursive <- boost_tree(
        mode = "regression",
        learn_rate = 0.35
    ) %>%
    set_engine("xgboost") %>%
