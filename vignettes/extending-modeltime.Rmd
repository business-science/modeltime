...
vignette: >
  %\VignetteIndexEntry{Extending Modeltime}
...
The `modeltime` package was with extensibility in mind. It's impossible to incorporate the entire [R time series ecosystem](https://cran.r-project.org/web/views/TimeSeries.html) into a single package. More than likely your going to need something that hasn't been included yet. And, if you're a developer, ___I challenge you___ to extend `modeltime` to suit your needs. I've spent a lot of time thinking about:
...
__Note - I've added a `seasonal_decomp()` function that includes the functionality we are about to create. Check that function out to see more about seasonal decomposition.__
...
### Step 5: Fit Indicators

We need to modify how parnsip handles features because date and date-time information cannot be processed by `model.frame()` (default behavior). This step will simplify once parsnip is updated. 

__Current Method (Not Desirable)__: We have to modify the `fit()` method to set `indicators = FALSE`, which is necessary to prevent dates from being converted to numeric features. 
...
__New Method (Best Approach)__: Coming soon to `parsnip (>0.1.1)`. 
...
We'll split the data into training and test sets. Then create a model on the training set. 
...
We'll refit on the full dataset and forecast forward 1 week. 
