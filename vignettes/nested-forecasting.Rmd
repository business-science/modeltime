...
1. __Global Modeling:__ Best for scalability using a Panel Data structure. See [Forecasting Panel Data](https://business-science.github.io/modeltime/articles/modeling-panel-data.html). 
...
The core idea of __nested forecasting__ is to convert a dataset containing may time series groups into a nested data set, then apply many models to each of the datasets. This allows for selection of the best models for the unique time series groups, rather than a single global model that performs best on average.

![Nested Modeltime Forecasting](modeltime-nested.jpg)
...
Next, we move into the __Nested Modeltime Workflow__ now that nested data has been created. The Nested Modeltime Workflow includes 3 key functions:
...
3. __Model Refitting:__ This is the final fitting stage where we fit to _actual data_. The _future forecast_ is generated from this step. 
