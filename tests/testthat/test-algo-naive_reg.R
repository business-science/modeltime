...
testthat::test_that("NAIVE - Multiple Time Series (Panel uses ID)", {
    ...
    future_last_series <- future_forecast_panel_tbl %>%
        filter(is.na(.model_id)) %>%
        filter(id == "M1") %>%
        slice_tail(n = 12) %>%
        pull(value)
    ...
    expect_equal(model_fit_panel$fit$extras$period, 12)
    expect_equal(nrow(future_forecast_panel_tbl), 1814)
    expect_equal(future_forecast_vec, rep_len(future_last_series, 60))
})
